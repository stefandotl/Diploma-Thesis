# -*- coding: utf-8 -*-
"""
Created on Thu Mar 12 16:42:08 2020
...
def get_contours_list(image):
    """searches the contours in the given image, convert them to a list and retruns the list"""
    ...
    for contour in contours:
        ...
        # convert numpay array to a list by adding every Point to a new list.
        for point in hull:
...
def plot_ellipse(contour, img_out):
    ...
    # filter_shape(contour, file_path_name)
...
def realistic_size(contour):
    ...
    if ratio > 1 or ratio < 0.39:   # 0.37 # real: 0.4 bis 0.99
        # print('ratio', ratio)
        return False
    ...
    if contour_area > ellipse_area or contour_area < sv.min_area_percent * ellipse_area:
        return False
...
def filter_points(contour_list):
    ...
    for contour in contour_list:
        ...
        while i < (len(contour)-1):
            ...
            # when Points are close to each other, the Points will NOT get filtered.
            if dist < sv.dist:
                cnt_list.append(contour[i])
                i = i + 1

            # when the distance is to large the upcoming point is filtered and the next neighbour point gets checked.
            else:
                i = i + 1

                # checks if the filtered point has some neighbour points and filters them also,
                # but maximum 2 points
                dist_fil = 0
                j = i
                max_points = 1
